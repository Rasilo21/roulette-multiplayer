(() => {
  ...
  const vscLog = (label, data) => { try { if (MP && MP.socket) MP.socket.emit('dbg:log', { label, data }); } catch {} };
  ...
  $('btn-spin').onclick = async ()=>{
    if (MODE==='mp') {
      if(MP.socket && MP.code){ try{ MP.socket.emit('mp:requestSpin', { code: MP.code }); }catch{} }
      return;
    }
  ...
  window.addEventListener('load', () => {
    ...
    function renderHud(state) {
      ...
      state.players.forEach(p => { 
        ...
        d.innerHTML = `
          <span class="dot"></span>
          <span class="name${MP.leader ? ' grantable' : ''}" title="${MP.leader ? 'Dar +1000 a este jugador' : ''}">${p.name}${p.id === state.leaderId ? ' 👑' : ''}</span>
          <span class="bal">${fmt(p.balance)}</span>
        `;
        hud.appendChild(d); 
      });

      // Líder puede clicar el nombre para dar +1000
      if (MP.leader) {
        document.querySelectorAll('#playersHud .player-pill').forEach(pill => {
          ...
          nameEl.title = 'Dar +1000 a este jugador';
          nameEl.addEventListener('click', (ev) => {
            ev.stopPropagation();
            try { MP.socket.emit('mp:grant100', { code: MP.code, playerId: pid }); safeLog('Líder: +1000 a '+pid); } catch{}
          });
        });
      }
    }

    function updateSpinGating(){
      const btn=$('btn-spin'); if(!btn) return;
      if (MODE==='mp' && MP.state){
        const allReady = !!MP.state.allReady;
        const isLeader = MP.leader;
        const isRound  = !!MP.state.roundActive;
        btn.disabled = !(allReady && isLeader && !isRound);
    ...
    function connectSocket(){
      ...
      let url; // same-origin en producción/Render
      // Si en local tienes front en 10000 y back en 3000, puedes descomentar:
      ...
      try {
        MP.socket = url ? io(url, opts) : io(opts);
      } catch (e) {
      ...
      if (!MP._wired){
        ...
        MP.socket.on('mp:ack', (payload) => {
          try {
            if (payload?.action === 'grant1000' && payload.ok && MP.leader) {
              toast(`+1000 € añadidos`);
            }
          } catch {}
        });
        ...
        MP.socket.on('mp:spin', async ({ number })=>{ 
          ...
          try { MP.socket.emit('mp:landed', { code: MP.code }); } catch {}
        });
        ...
        MP.socket.on('mp:req:100', ({ playerId, name })=>{
          try {
            ...
            island.textContent = `Solicitud +1000: ${name}`;
            island.onclick = () => {
              try { MP.socket.emit('mp:grant100', { code: MP.code, playerId }); safeLog(`MP(líder): +1000 otorgado a ${name}`); } catch{}
    ...
    if (btnCreate){
      btnCreate.onclick = () => {
        MODE='mp';
        const name = (document.getElementById('mp-name-create')?.value||'').trim()||'Jugador';
        const privacy = document.getElementById('mp-privacy')?.value||'public';
        const s = connectSocket(); if(!s){ toast('No se pudo iniciar socket'); return; }
        try{ s.emit('mp:createRoom', { name, privacy }); }catch(e){ safeLog('emit error createRoom: '+(e?.message||e)); }
      };
    }
    if (btnJoin){
      btnJoin.onclick = () => {
        MODE='mp';
        const name = (document.getElementById('mp-name-join')?.value||'').trim()||'Jugador';
        const code = (document.getElementById('mp-room-code')?.value||'').trim();
        if(!code){ toast('Ingresa código de sala'); return; }
        const s = connectSocket(); if(!s){ toast('No se pudo iniciar socket'); return; }
        try{ s.emit('mp:joinRoom', { name, code }); }catch(e){ safeLog('emit error joinRoom: '+(e?.message||e)); }
      };
    }
    if (btnCopyCreated){
      btnCopyCreated.onclick = async () => {
        const code = codeSpan?.textContent?.trim(); if(!code) return;
        try { await navigator.clipboard.writeText(code); toast('Código copiado'); }
        catch {
          const tmp=document.createElement('input'); tmp.value=code; document.body.appendChild(tmp);
          tmp.select(); document.execCommand('copy'); tmp.remove(); toast('Código copiado');
        }
      };
    }
...
const Wheel2D = (() => {
  ...
  function draw() {
    ...
    for (let i=0;i<37;i++){
      ...
      const aSep = (-90 + (i+1)*STEP - sep) * Math.PI/180;

      ctx.beginPath(); ctx.moveTo(cx,cy); ctx.arc(cx,cy,R,a0,aSep); ctx.closePath();
      ctx.fillStyle = (n===0) ? '#18a252' : (REDS.has(n) ? '#c1121f' : '#171b24'); ctx.fill();

      ctx.beginPath(); ctx.moveTo(cx,cy); ctx.arc(cx,cy,R,aSep,a1); ctx.closePath();
      ctx.fillStyle = '#0a0f15'; ctx.fill();
    }
    ...
    ctx.font = `${Math.max(10, Math.floor(R*0.08))}px system-ui, -apple-system, Segoe UI, Inter, Roboto, Arial`;
    for (let i=0;i<37;i++){
      const n = ORDER[i];
      ...
      ctx.fillStyle = (n===0) ? '#eaffea' : '#f6f8ff'; ctx.fillText(String(n), 0, 0);
      ctx.restore();
    }
  }

  // Útil para mapear ángulo a índice ORDER
  const angleToIndex = (deg) => {
    const normalizedAngle = norm(deg) % 360 + 270;
    const rel = (270 - normalizedAngle + 360) % 360;
  ...
  function __crandFloat(){
    const g = (window.crypto||window.msCrypto); if (!g) return Math.random();
    const arr = new Uint32Array(1); g.getRandomValues(arr);
    return (arr[0] >>> 0) / 0x100000000;
  }

  function spin() {
    // Singleplayer: total de grados aleatorio (vueltas + jitter) => número uniforme
    const fastTurns = 4 + __crandInt(5);      // 4..8
    const extraTurns = __crandInt(4);         // 0..3
  ...
  function spinTo(number){
    ...
    const extraTurns = 4 + __crandInt(6);     // 4..9 vueltas extra
    ...
    return new Promise(res => {
      ...
      const onEnd = () => {
        $c.removeEventListener('transitionend', onEnd);
        currentRot = final;
        res(number); // aterriza exactamente en ese número
      };
      $c.addEventListener('transitionend', onEnd, { once: true });
    });
  }
  ...
  function init(selector = '#wheel2d'){
    const c = document.querySelector(selector); if (!c) return console.warn('[Wheel2D] No se encontró el canvas');
    $c = c;
    ctx = $c.getContext('2d'); sizeCanvas(); draw();
